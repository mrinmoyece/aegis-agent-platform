...
> **Current status:** Layers 1–2 provide boundaries and identity/governance.
> Layer 3 implements the durable PostgreSQL event ledger, inbox/outbox,
> projections, replay, RLS-backed repositories, and authorized inspection.
> Worker, model, connector, tool, and agent-execution modules remain planned.
...
| Failure modes and runbooks (`failure-modes.md`, `runbook.md`) | Diagnose crashes, stale leases, partial effects, provider faults, poisoned evidence, and regional failure | 3–8 | Layer 3 storage failures implemented; worker/provider/region sections planned |
...
| Hands-on labs (`labs.md`) | Turn each invariant into executable evidence and inject realistic failures | 1–8 | Layers 1–3 tests runnable |
